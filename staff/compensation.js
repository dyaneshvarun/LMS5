...
$(document).ready(function(){
	...
	$.post("qengine.php",{
		...
		var res = $.parseJSON(data);
		$.each(res,function(i,obj){
			var date1 = new Date(obj.CDATE);
			var parseDate =  $.datepicker.formatDate("dd/mm/yy",date1);
			compDates.push(parseDate);
			cleaveid.push(obj.LEAVEDATE_ID);
		});
	...
	$("#date1,#vacation").change(function(){
		...
		for (var k=0;k<nod;k++)str+=compDates[k]+" , ";
		$("#compDateTab").val(str);
			...
			$.post("qengine.php",{
				...
				if(data == 1){
					$.post("../testMail.php",{
						...
						subject : "Leave " + ltype+ "- Applied on  "+today ,
				...
				if(data == 2){
					...
					$.post("../testMail.php",{
						content : HODContent ,
						subject : "Review Leave " + ltype+ "- Applied on  "+today ,
						toEmail : "hod@auist.net", //HOD Mail ID
						toName  : "HOD"  //Name of the corresponding Email
					},function(data,status){
						//alert(data);
					});;
					var StaffContent = " You have applied "+ ltype +"(Leave Type) leave for "+$("#nod").val() +"day(s) from "+fdate+" to "+tdate +". "
