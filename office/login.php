	...
	if(isset($_POST['op'])){
		...
		switch($op){
			case 1:
			{
				...
				$hashpass = md5($pass);
				if(!$res = mysqli_query($conn,"CALL CHECK_OFFICE_LOGIN('$email','$hashpass',@ADMI_ID)")){
					echo mysqli_error($conn);
				}
				if(!$res = mysqli_query($conn,"SELECT @ADMI_ID AS STA_ID")){
...
<head>
    ...
    <title>IST Leave System | Office Portal Login</title>
